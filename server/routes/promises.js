...
router.put('/:id', wrap(async (req, res) => {
  ...
  if (!upd) return res.json(await q1('SELECT * FROM promises WHERE id = $1', [id]))
  const sql = `UPDATE promises SET ${upd.clause}, updated_at = to_char(now(), 'YYYY-MM-DD"T"HH24:MI:SS')
               WHERE id = $${upd.nextIndex} RETURNING *`
  res.json(await q1(sql, [...upd.values, id]))
}))

// DELETE /promises/:id  (soft delete)
router.delete('/:id', wrap(async (req, res) => {
  await q(`UPDATE promises SET status = 'deleted' WHERE id = $1`, [Number(req.params.id)])
  res.json({ ok: true })
}))
